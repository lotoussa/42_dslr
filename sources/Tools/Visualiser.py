...
class Visualiser(ArgParser):
    ...
    def _pair_plot_visualizer(self, header, hue="Hogwarts House", map_lower=False):
        g = sns.pairplot(self.raw_data, hue=hue, vars=header, plot_kws={"edgecolor": "none"})
    ...
    def _create_scatter(
        self, x, y=None, title="Scatter", label=None, xlabel="x", ylabel=None, yticks=None
    ):
        fig = plt.figure()
    ...
    def _scatter_plot_visualizer(self, header):
        ...
        figures.append(
            self._create_scatter(
                filtered_data, title="Two similar features", label=header, xlabel="Marks"
            )
        )
